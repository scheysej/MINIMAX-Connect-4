# The purpose of this program is a connect 4 game where you play against an AI agent that uses minimax alpha beta pruning to 
# make its moves. This code has an improved heuristic and uses a max depth of 5 for best performance.
# Created by Ethan Scheys on 03/18/2025

# Game Stats
# 1)  AI
# 2)  ME
# 3)  ME (i found a hack, that im going to try to fix)
# 4)  AI (increased max depth to 5)
# 5)  AI
# 6)  AI
# 7)  AI
# 8)  AI (changed win and block win to 1000. although it seems to miss wins, I can't seem to win)
# 9)  AI
# 10) AI
...
def evaluate_window(window, piece):
    ...
    # Base Score, then improve on it
    score = window.count(piece) - window.count(opp_piece)
    ...
    # Halfway, 2 filled with 2 empty
    elif window.count(piece) == 2 and window.count(0) == 2:
        score += 2
    
    # Almost winning for the human user
...
def iterative_deepening_minimax(board, max_time, is_maximizing):
    ...
    while True:
        if time.time() >= time_limit or depth >= 5:
